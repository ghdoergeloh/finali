...
pnpm -F @finali/api dev             # Start only the API
pnpm -F react dev                 # Start only the React frontend

# Build & Type Check
pnpm build                        # Build all workspaces
pnpm typecheck                    # Type-check all workspaces
pnpm -F @finali/db build            # Build a single package (use -F <package-name>)
...
pnpm -F @finali/auth generate       # Regenerate auth tables into packages/db/src/schema/auth-schema.ts
...
pnpm -F @finali/ui ui-add           # Add a new UI component
```
...
- **Imports**: Auto-sorted by Prettier — order is: types, react, third-party, `@finali/*` workspace packages, local (`~/`, `../`, `./`). Tailwind classes sorted in `cn()` and `cva()` calls.
...
Design tokens live in `tooling/tailwind/theme.css` (CSS variables + `@theme inline` + class‑based `dark` variant). `packages/ui` and all apps consume them via `@finali/tailwind-config/theme`.
